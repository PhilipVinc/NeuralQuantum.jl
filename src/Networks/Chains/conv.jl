# This file is copied from Flux.jl
...
function Conv(k::NTuple{N,Integer}, ch::Pair{<:Integer,<:Integer}, σ = identity;
  ...
  filt = init(k..., ch...)
  Conv(filt, zeros(eltype(filt), ch[2]), σ,
       stride = stride, pad = pad, dilation = dilation)
end
#@functor Conv

functor(c::Conv) = (weight = c.weight, bias = c.bias),
  (w,b) -> Conv(c.σ, w, b, c.stride, c.pad, c.dilation)

function (c::Conv)(x::AbstractArray)
  # TODO: breaks gpu broadcast :(
  # ndims(x) == ndims(c.weight)-1 && return squeezebatch(c(reshape(x, size(x)..., 1)))
  if ndims(x) == ndims(c.weight) - 2
...
CrossCor(k::NTuple{N,Integer}, ch::Pair{<:Integer,<:Integer}, σ = identity;
     init = glorot_uniform, stride = 1, pad = 0, dilation = 1) where N = begin
  filter = init(k..., ch...)
  CrossCor(filter, zeros(eltype(filter), ch[2]), σ,
       stride = stride, pad = pad, dilation = dilation)
end
